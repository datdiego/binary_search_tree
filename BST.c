 ...
 * 2. Process the node in the left sub-tree.
 * 3. Proccess the nodes in the right subtree.
 * 
 ...
 * Returns -1 if tree is emtpy, and 1 if root is not NULL.
 * 
 * The operation is done recursively.
 * 
 * @param nodeT *p
...
int getHeight(nodeT *p){
    ...
    if(p == NULL){
        ...
        return ( 1 + ((getHeight(p->pLeft) > getHeight(p->pRight)) ? getHeight(p->pLeft) : getHeight(p->pRight) ) );
    }
 ...
 * Returns -1 if tree is emtpy or the sum of the rest of the nodes.
 * 
 * The operation is done recursively.
 * 
 * @param nodeT *p
 ...
 * getNumberOfLeaves();
 * 
 * Base case 1: The subtree is NULL and contains no leaves, return 0.
 * Base case 2: If the subtree is not NULL, and contains no children, return 1;
 * Then recursively check the rest of the tree.
 * 
 * The operation is done recursively.
 * 
 * @param nodeT *p
...
int getNumberOfLeaves(nodeT *p){
    ...
    if(p == NULL){
        return 0;
    } else if ( p != NULL && p->pLeft == NULL && p->pLeft == NULL){
        return 1;
    } else {
